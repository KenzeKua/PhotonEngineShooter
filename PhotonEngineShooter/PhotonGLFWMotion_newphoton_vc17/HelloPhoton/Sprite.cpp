...
float Sprite::GetHalfwidth()
{
	return halfWidth;
}
float Sprite::GetHalfheight()
{
	return halfHeight;
}
...
{	
	halfWidth = m_width * 0.5f * scale.mVal[0];
	halfHeight = m_height * 0.5f * scale.mVal[1];
	
	float displacementX = halfWidth * cos (radian) - halfHeight * sin (radian);
	float displacementY = halfWidth * sin (radian) + halfHeight * cos (radian);
